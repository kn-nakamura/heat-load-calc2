// Piloti floor master data form

import { Box, TextField, Button, Typography, Divider } from '@mui/material';
import Grid from '@mui/material/Grid';
import { Save as SaveIcon, Delete as DeleteIcon } from '@mui/icons-material';
import { useState, useEffect } from 'react';
import { PilotiFloorMaster } from '../../types';
...
interface PilotiFloorFormProps {
  selectedId: string | null;
}

export const PilotiFloorForm: React.FC<PilotiFloorFormProps> = ({ selectedId }) => {
  const { pilotiFloors, materials, updatePilotiFloor, deletePilotiFloor } = useMasterDataStore();
  const { showSnackbar } = useUIStore();

  const selectedItem = pilotiFloors.find((item) => item.id === selectedId);

  const [formData, setFormData] = useState<Partial<PilotiFloorMaster>>({
    name: '',
    floorType: 'ピロティ床',
    layers: [],
  ...
  const handleSave = async () => {
    ...
    try {
      updatePilotiFloor(selectedId, formData);
  ...
  return (
    <Box sx={{ p: 3, height: '100%', overflow: 'auto' }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        <Typography variant="h6">ピロティ床</Typography>
