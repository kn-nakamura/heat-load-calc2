// Earth floor master data form

import { Box, TextField, Button, Typography, Divider } from '@mui/material';
import Grid from '@mui/material/Grid';
import { Save as SaveIcon, Delete as DeleteIcon } from '@mui/icons-material';
import { useState, useEffect } from 'react';
import { EarthFloorMaster } from '../../types';
...
interface EarthFloorFormProps {
  selectedId: string | null;
}

export const EarthFloorForm: React.FC<EarthFloorFormProps> = ({ selectedId }) => {
  const { earthFloors, materials, updateEarthFloor, deleteEarthFloor } = useMasterDataStore();
  const { showSnackbar } = useUIStore();

  const selectedItem = earthFloors.find((item) => item.id === selectedId);

  const [formData, setFormData] = useState<Partial<EarthFloorMaster>>({
    name: '',
    floorType: '土間床',
    layers: [],
  ...
  const calculateThermalProperties = () => {
    ...
    // Earth floor: only interior surface resistance (no exterior)
  ...
  const handleSave = async () => {
    ...
    try {
      updateEarthFloor(selectedId, formData);
  ...
  return (
    <Box sx={{ p: 3, height: '100%', overflow: 'auto' }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        <Typography variant="h6">土間床</Typography>
