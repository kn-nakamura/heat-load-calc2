// Interior wall master data form

import { Box, TextField, Button, Typography, Divider } from '@mui/material';
import Grid from '@mui/material/Grid';
import { Save as SaveIcon, Delete as DeleteIcon } from '@mui/icons-material';
import { useState, useEffect } from 'react';
import { InteriorWallMaster } from '../../types';
...
interface InteriorWallFormProps {
  selectedId: string | null;
}

export const InteriorWallForm: React.FC<InteriorWallFormProps> = ({ selectedId }) => {
  const { interiorWalls, materials, updateInteriorWall, deleteInteriorWall } = useMasterDataStore();
  const { showSnackbar } = useUIStore();

  const selectedItem = interiorWalls.find((item) => item.id === selectedId);

  const [formData, setFormData] = useState<Partial<InteriorWallMaster>>({
    name: '',
    wallType: '内壁',
    layers: [],
  ...
  const calculateThermalProperties = () => {
    ...
    // Interior wall: surface resistance on both sides
  ...
  const handleSave = async () => {
    ...
    try {
      updateInteriorWall(selectedId, formData);
  ...
  return (
    <Box sx={{ p: 3, height: '100%', overflow: 'auto' }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        <Typography variant="h6">内壁</Typography>
