// Roof master data form

import { Box, TextField, Button, Typography, Divider } from '@mui/material';
import Grid from '@mui/material/Grid';
import { Save as SaveIcon, Delete as DeleteIcon } from '@mui/icons-material';
import { useState, useEffect } from 'react';
import { RoofMaster } from '../../types';
...
export const RoofForm: React.FC<RoofFormProps> = ({ selectedId }) => {
  const { roofs, materials, updateRoof, deleteRoof } = useMasterDataStore();
  const { showSnackbar } = useUIStore();

  const selectedItem = roofs.find((item) => item.id === selectedId);

  const [formData, setFormData] = useState<Partial<RoofMaster>>({
    name: '',
    roofType: '屋根',
    layers: [],
  ...
  const handleSave = async () => {
    ...
    try {
      updateRoof(selectedId, formData);
  ...
  return (
    <Box sx={{ p: 3, height: '100%', overflow: 'auto' }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        <Typography variant="h6">屋根</Typography>
